...
public class CreateDynamicTabDtoValidator : AbstractValidator<CreateDynamicTabDto>
{
    public CreateDynamicTabDtoValidator()
    {
        RuleFor(x => x.Name)
            .NotEmpty()
            .MaximumLength(100)
        ...
        RuleFor(x => x.TabType)
            .NotEmpty()
            .Must(x => new[] { "letter", "database", "upload" }.Contains(x.ToLower()))
            .WithMessage("TabType must be either 'letter', 'database', or 'upload'");

        RuleFor(x => x.DataSource)
            .NotEmpty()
            .Must(x => new[] { "excel_upload", "database_query", "api", "manual", "form" }.Contains(x.ToLower()))
